...
Use this in a clean Chromium profile with the extension installed, the popup available, and the local ASR service expected on `localhost:8000`. Run the cases in order and return the app to a known-good state before moving on.
...
- Recovery: start WhisperLiveKit on this machine, confirm `localhost:8000` is free, then retry discovery or refresh the popup.
...
- Recovery: reload the extension, accept the prompt, and retry the readiness check.
...
- Recovery: wait for the local service to recover, refresh diagnostics, then restart captions if needed.
...
- Verify: the sidebar export flow fails while clipboard copy stays separate.
- Recovery: retry after the local service is reachable and the transcript has settled.
...
- Use the same recovery wording in bug reports and support notes.
